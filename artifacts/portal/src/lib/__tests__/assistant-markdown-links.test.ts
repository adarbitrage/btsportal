...
describe("normalizeAssistantLinks", () => {
  ...
  it("leaves correctly formatted links untouched", () => {
    const good = "Start with [The Blitz](/blitz) and [Your Second Engine](/core-training/pillars-to-blitz).";
    expect(normalizeAssistantLinks(good)).toBe(good);
  });

  it("handles multi-segment nav paths", () => {
    expect(
      normalizeAssistantLinks("Your Second Engine ([/core-training/pillars-to-blitz](/core-training/pillars-to-blitz))"),
    ).toBe("[Your Second Engine](/core-training/pillars-to-blitz)");
  });
