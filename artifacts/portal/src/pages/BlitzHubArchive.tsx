...
const STEP_COURSE_PREFIX = "blitz-archive-hub-step-";
const API_BASE = `${import.meta.env.BASE_URL}api`;
const GUIDE_BASE = `${import.meta.env.BASE_URL}blitz-archive/guide`;
...
export default function BlitzHubArchive() {
    ...
    fetch(`${API_BASE}/course-progress`, { credentials: "include" })
      ...
      .then((rows: ProgressEntry[]) => {
        ...
        for (const row of rows ?? []) {
          const m = row.courseId?.match(/^blitz-archive-hub-step-(\d+)$/);
