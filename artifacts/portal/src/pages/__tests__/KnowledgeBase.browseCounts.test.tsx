...
// BrowseLanding renders wouter <Link> for the "Start here" lessons and Blitz
// phases; stub it so we don't need a Router context.
vi.mock("wouter", () => ({
...
function cardFor(label: string): HTMLElement {
  // The category card is the <button> whose accessible name contains the label.
...
describe("BrowseLanding category counts", () => {
  it("renders 'N articles' beneath each category from the counts map", () => {
    ...
    render(<BrowseLanding onSelectCategory={vi.fn()} counts={counts} />);
  ...
  it("renders a zero count for categories absent from the counts map", () => {
    // counts present but missing some categories -> those fall back to 0.
  ...
  it("degrades gracefully when counts are absent (null), omitting the count line", () => {
    render(<BrowseLanding onSelectCategory={vi.fn()} counts={null} />);

    // The category cards still render...
    expect(cardFor(CATEGORY_LABELS.blitz)).toBeInTheDocument();
    // ...but no "N article(s)" text is shown anywhere while counts load.
