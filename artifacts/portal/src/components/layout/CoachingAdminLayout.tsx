...
const coachingAdminNav = [
  { href: "/admin/coaching", label: "Coach Management", icon: LayoutDashboard },
  { href: "/admin/coaching/availability", label: "Availability", icon: Calendar },
  { href: "/admin/coaching/overrides", label: "Overrides", icon: CalendarOff },
  { href: "/admin/coaching/sessions", label: "Sessions", icon: ClipboardList },
  { href: "/admin/coaching/notes", label: "Session Notes", icon: FileEdit },
  { href: "/admin/coaching/analytics", label: "Analytics", icon: BarChart3 },
];

export function CoachingAdminLayout({ children }: { children: ReactNode }) {
        ...
        <div className="p-6 border-b border-border/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground tracking-widest uppercase">1-on-1 Coaching</p>
            </div>
          </div>
        </div>

        <div className="flex-1 py-6 px-4 space-y-1 overflow-y-auto">
          {coachingAdminNav.map((item) => {
            const isActive = location === item.href || (item.href !== "/admin/coaching" && location.startsWith(item.href));
            return (
