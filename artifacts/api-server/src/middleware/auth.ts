...
export function authenticate(req: Request, res: Response, next: NextFunction): void {
  ...
  if (PUBLIC_PATHS.some(p => path === p) || path.startsWith("/v1/marketing/") || path.startsWith("/api/webhooks/") || path.startsWith("/webhooks/") || path.startsWith("/go/") || path.startsWith("/ops/") || (process.env.NODE_ENV !== "production" && (path.startsWith("/api/dev/") || path.startsWith("/dev/")))) {
    next();
    return;
  }
