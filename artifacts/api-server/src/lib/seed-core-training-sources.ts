...
import { aiSourceDocumentsTable } from "@workspace/db/schema";
 ...
 *
 * Loads the two core-training prose bodies into `ai_source_documents` as
 ...
 *
 * Both bodies file into the `reference_docs` folder with the `curriculum`
 ...
 * via the Transcript Cleaner pipeline (a separate, human-gated flow); the
 * section-anchored Blitz reference docs are generated by the (manually run)
 * blitz-section-docgen pipeline — this seeder deliberately no longer mirrors
 * `blitz_lessons` into the source corpus; and the synthesis run / review /
 * publish / coverage steps are runtime + human-gated (they need a live LLM),
 * so they are intentionally out of this boot seed.
 */
 ...
 * re-authored prose bodies. (The per-lesson `blitz_lessons` mirror was retired
 * with the section-anchored reference-doc rebuild — the curriculum now enters
 * the corpus via the blitz-section-docgen pipeline instead.)
 */
export async function buildCoreTrainingSourceDocs(): Promise<CoreTrainingSourceDoc[]> {
  return PROSE_DOCS.map((doc) => ({
    title: doc.title,
    content: doc.content,
