 ...
 * from server startup (bootstrap-critical-prerequisites.ts) and from tests.
 * The CLI entrypoint lives in ../scripts/rescrub-knowledgebase-docs.ts, which
...
export interface TitleLeak {
  id: number;
  title: string;
}

/**
 * One-time cleanliness check: scan every knowledgebase_docs title and return
 * the rows whose title would STILL change if scrubbed — i.e. a coach surname
 * (or other private token) is still present. The privacy filter is the single
 * source of truth for what "clean" means, so a title is considered clean iff
 * `scrubPrivateContent(title).trim() === title`.
 *
 * After rescrubKnowledgebaseDocs() has run this MUST be empty. It is the
 * verification the deploy/post-merge path uses to confirm production titles are
 * clean and to surface a filter gap loudly if one is ever found.
 */
export async function findUnscrubbedTitles(): Promise<TitleLeak[]> {
  const rows = await db
    .select({
      id: knowledgebaseDocsTable.id,
      title: knowledgebaseDocsTable.title,
  ...
  const leaks: TitleLeak[] = [];
  for (const row of rows) {
    if (scrubPrivateContent(row.title).trim() !== row.title) {
      leaks.push({ id: row.id, title: row.title });
    }
  }
  return leaks;
}
