...
describe("voice assistant retrieval honors the citable gate (primary path)", () => {
  beforeAll(async () => {
    ...
    // All three docs are seeded DIRECTLY into ai_live_documents (Task #2029:
    // the legacy table + boot mirror are retired), so the citable gate on the
    // retrieval side is what must exclude the transcript + unverified docs.
  });
