...
// forgot-password handler many times without bumping into the per-IP cap.
vi.mock("../lib/redis", () => ({
  getRedis: () => null,
...
describe("POST /api/auth/forgot-password Turnstile verification", () => {
  it("bypasses verification when TURNSTILE_SECRET_KEY is unset", async () => {
    // The handler responds with a generic message regardless of whether the
    // email exists. If captcha rejected the request we'd get a structured
    // CAPTCHA_REQUIRED error instead.
  ...
  it("rejects with 400 CAPTCHA_REQUIRED when the token is an empty/whitespace string", async () => {
    ...
    const res = await request(app)
      .post("/api/auth/forgot-password")
      .send({ email: "anyone@example.com", captchaToken: "   " });
  ...
  it("rejects with 400 CAPTCHA_INVALID when Turnstile says the token failed", async () => {
    ...
    const res = await request(app)
      .post("/api/auth/forgot-password")
      .send({ email: "anyone@example.com", captchaToken: "bad-token" });
  ...
  it("rejects with 400 CAPTCHA_INVALID when the verification call throws", async () => {
    ...
    const res = await request(app)
      .post("/api/auth/forgot-password")
      .send({ email: "anyone@example.com", captchaToken: "any-token" });
  ...
  it("forwards the request to the handler when the token verifies successfully", async () => {
    ...
    const res = await request(app)
      .post("/api/auth/forgot-password")
      .send({ email: "anyone@example.com", captchaToken: "good-token" });

    // When captcha verifies, the handler returns the same generic 200 it
    // returns in the no-secret case — proving the middleware called next()
