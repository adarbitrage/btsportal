...
// login handler many times without bumping into the per-IP cap.
vi.mock("../lib/redis", () => ({
  getRedis: () => null,
...
describe("POST /api/auth/login Turnstile verification", () => {
  it("bypasses verification when TURNSTILE_SECRET_KEY is unset", async () => {
    // With no email/password the handler 400s with the legacy
    // `{ error: "Email and password are required" }` body. If the captcha
    // middleware rejected the request we'd see a structured CAPTCHA_REQUIRED
    // error instead.
  ...
  it("forwards the request to the handler when the token verifies successfully", async () => {
    ...
    // For an unknown email + valid captcha, the login handler returns the
    // generic 401 invalid-credentials response — proving the middleware
    // called next() rather than short-circuiting the response.
    const res = await request(app)
      .post("/api/auth/login")
      .send({
