...
Which means an image with a white or washed-out light background dissolves into the page. It has no edges. It has no presence. It has nothing to separate the picture from the frame, so the eye slides across it the way it slides across empty margin, and it vanishes at a glance. This is the single most common color mistake in ad creatives, and it is disqualifying. It does not matter what else the image has going for it — a compelling subject, a great moment-of-use shot, a perfect angle match — a white background erases all of it before the reader's eye ever registers there was something to see.
...
- **Jump page (bridge page) — congruent, with a minor allowance.** The bridge page carries the curiosity the ad opened toward the play button for a reader who is only one click past the feed. It sits off the feed, so there's no feed to camouflage into and a slightly more composed color treatment won't hurt (*see Document 5, The UGC Default*, on that minor permission). But "composed" is not a license to go white-background catalog. The palette still agrees with the angle, backgrounds still carry depth and color, and the imagery still points the reader toward the next step rather than announcing itself as branded design.
