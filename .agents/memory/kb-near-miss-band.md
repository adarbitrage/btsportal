...
- **Voice extension measured and found structurally impossible** (voice's entire retrieval scope = operations root = the band's absolute exclusion → band-eligible score always 0; zero voice content-gap rows dev+prod; spoken deep-content phrasings score 0.13–0.24 vs voice's ops corpus). Voice stays binary; kb-voice-near-miss-calibration.test.ts pins this and is the alarm if voice's scope ever widens — only then design a TTS hedge contract + recalibrate. Don't re-attempt a voice opt-in while the ops exclusion and ops-only scope both stand.
