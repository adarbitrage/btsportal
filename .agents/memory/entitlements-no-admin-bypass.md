...
**The `coach` role reuses this exact pattern.** Coaches get the full member experience via the SAME three consumer-layer bypasses (sidebar, EntitlementRoute, server guards) keyed on `isCoachRole` (single-sourced in lib/auth) — NOT by injecting entitlements (their real entitlement set stays empty; role-gated UI like the upgrade card is suppressed explicitly). **Why kept separate from `getIsAdmin`/`isAdminRole`:** coach must NEVER inherit admin or community-moderation powers, so the coach bypass is an independent `isAdminUser || isCoach` OR at each gate; moderation/permission checks stay admin-only. The bookable 1-on-1 roster (`session_pack_coaches`) is unrelated to this auth role — no auto-link.
