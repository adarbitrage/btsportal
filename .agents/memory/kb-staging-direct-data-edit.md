...
2026-07-27 addendum: live ai_live_documents rows can DIVERGE in wording from their staging/seed text (e.g. cross-reference tails rewritten at publish). Never assume the seed paragraph matches the stored row — fetch the exact stored text per row (live content, staging content AND edited_content separately) and build row-specific replace() strings; verify with a phrase-family regex sweep after each pass.
