...
-   **KB Training Document Pipeline:** Transcript-to-training-document pipeline at `/api/admin/knowledgebase/pipeline/*` and `/api/admin/knowledgebase/staging/*`. Processes 97 raw video transcripts through filler removal + GPT-4o structured extraction into clean training docs stored in `kb_staging_docs` table. Admin review UI at `/admin/chat/knowledgebase/review` with status filters (pending/approved/rejected/merged/pushed), inline editing, bulk approve, merge duplicates, and push-to-KB. Push rebuilds `training-documents.txt` from all approved+pushed docs (idempotent, non-destructive) and reloads knowledge base. All pipeline/staging endpoints require admin auth.
