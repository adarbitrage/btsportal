...
-   **KB Training Document Pipeline:** Transcript-to-training-document pipeline at `/api/admin/knowledgebase/pipeline/*` and `/api/admin/knowledgebase/staging/*`. Processes 97 raw coaching video transcripts + 81 Blitz training videos through filler removal + GPT-4o structured extraction into clean training docs stored in `kb_staging_docs` table. Blitz pipeline downloads videos from Vidalytics API, extracts audio via ffmpeg, transcribes with gpt-4o-mini-transcribe (with automatic chunking for >25min videos), matches to 93-lesson curriculum map (fuzzy matching), extracts structured docs with Blitz-specific prompts including phase/module/lesson metadata, and auto-rebrands TCE→BTS. Admin review UI at `/admin/chat/knowledgebase/review` with status filters, Blitz metadata badges (phase, module, lesson, network/publisher path), inline editing, bulk approve, merge duplicates, and push-to-KB. Push rebuilds `training-documents.txt` from all approved+pushed docs (idempotent, non-destructive) and reloads knowledge base. All pipeline/staging endpoints require admin auth.
