...
-   **UI/UX Design:** Features a BTS blue primary brand color, warm off-white background, Roboto font, editorial card layouts with warm borders, and distinct product badge coloring. Full mobile responsiveness is implemented with a hamburger menu, responsive typography, and mobile-first grid layouts.
-   **Authentication & Authorization:** Custom email/password authentication with JWTs (short-lived access, longer-lived refresh tokens with rotation), bcryptjs for password hashing, account lockout, email verification, password reset, and CSRF protection. Authorization uses granular `{resource}:{action}` permissions and Role-Based Access Control (RBAC).
-   **Entitlement System:** Access to content and features is determined by product purchases.
-   **Key Pages/Features:** Includes comprehensive authentication flows, a 5-step onboarding wizard, dynamic dashboards, Core Training with progress tracking, a detailed Blitz™ campaign guide, community feed, member directory, coaching scheduling, support center, and AI chat.
...
-   **Branding Rule:** All content from the Cherrington Experience website must be rebranded to "Build Test Scale" / "BTS," removing all previous branding and personal imagery.
...
-   **Security:** API Key authentication with granular permissions and Redis-backed sliding window rate limiting. All admin actions are logged to an `audit_log` table.
...
-   **AI Assistant:** Uses OpenAI GPT via Replit AI for conversations, SSE streaming, and RAG from a BTS knowledge base (Q&A, glossary, coaching/video transcripts).
-   **KB Training Document Pipeline:** Processes coaching videos (Vidalytics) and call transcripts (.docx) into structured knowledge base documents, including rebranding and metadata extraction. An admin review UI facilitates editing, approval, and pushing content to the knowledge base.
...
-   **Video Hosting/Transcription:** Vidalytics