/**
 * Image Foundations — series PDF renderer (clone of the proven Copywriting
 * Foundations pipeline, scripts/src/render-copywriting-foundations.ts).
 *
 * Renders each markdown doc in docs/image-foundations/ to a branded,
 * mobile-first PDF via an HTML/CSS series template printed with the system
 * (nix) Chromium. One command regenerates everything:
 *
 *   pnpm --filter @workspace/scripts run render-image-foundations-pdfs
 *
 * Or a single doc:
 *
 *   pnpm --filter @workspace/scripts run render-image-foundations-pdfs -- 03
 *
...
const DOCS_DIR = path.join(REPO_ROOT, "docs/image-foundations");
const OUT_DIR = path.join(
  REPO_ROOT,
  "artifacts/api-server/src/assets/image-foundations",
);
const FONTS_DIR = path.join(REPO_ROOT, "scripts/pdf-assets/fonts");

const SERIES_NAME = "Image Foundations";

/**
 * Committed-PDF filename overrides. Doc 2's markdown filename is deliberately
 * frozen (02-faces-gaze-and-hands.md — references depend on it), but the
 * member-visible series was retitled "The Attention Devices", so the committed
 * PDF must not carry the old title. All other docs keep md-derived names.
 */
const FILENAME_OVERRIDES: Record<string, string> = {
  "02-faces-gaze-and-hands.md": "02-the-attention-devices.pdf",
};
  ...
  try {
    ...
    for (const doc of docs) {
      ...
      const outFile = path.join(
        OUT_DIR,
        FILENAME_OVERRIDES[doc.file] ?? doc.file.replace(/\.md$/, ".pdf"),
      );
      const footer = `
